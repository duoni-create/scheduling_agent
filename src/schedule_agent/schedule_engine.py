...
def topological_sort(requirements: list[Requirement]) -> list[Requirement]:
    """拓扑排序处理需求依赖"""
    ...
    while queue:
        req_id = queue.pop(0)
        sorted_ids.append(req_id)
        for neighbor in adj[req_id]:
            in_degree[neighbor] -= 1
            if in_degree[neighbor] == 0:
                queue.append(neighbor)

    if len(sorted_ids) != len(requirements):
        raise ValueError("检测到循环依赖，无法排期")

    return [req_map[req_id] for req_id in sorted_ids]
...
def find_earliest_plan(
...
) -> tuple[date, str, date, str, str] | None:
    """找到最早的排期方案，返回 (start_date, start_half, end_date, end_half, owner)"""
    ...
    for candidate in candidates:
        ...
        if best_end is None or end_score < best_end:
            best_end = end_score
            best_plan = (start_date, start_half, end_date, end_half, candidate.name)
...
def schedule_requirements(
    ...
    # 拓扑排序
    sorted_reqs = topological_sort(requirements)
    sorted_reqs = sort_requirements(sorted_reqs, strategy)
    ...
    for req in sorted_reqs:
        ...
        for subtask in subtasks:
            ...
            start_date_val, start_half_val, end_date_val, end_half_val, owner = plan

            # 标记占用
            slots_needed = int(subtask["days"] * 2)
            current_day, current_half = start_date_val, start_half_val
            for _ in range(slots_needed):
                occupied_slots[(owner, current_day, current_half)] = True
                current_day, current_half = next_half_day(current_day, current_half)
